...
topic: AI 工程 # AI 工程 | 前端开发 | 效率工具 | AI 日报
tags:
  - Agent
  - 工具调用
draft: true
...
AI 新闻内容统一归入 `AI 日报` 专题：

- 日报文件名：`ai-daily-YYYY-MM-DD.md`
- 周报文件名：`ai-weekly-YYYY-MM-DD.md`，日期使用该周周日
- 日报标题：`AI 日报｜YYYY 年 M 月 D 日`
- 周报标题：`AI 周报｜M 月 D 日—M 月 D 日`
- 日报标签至少包含 `AI 日报`，周报标签至少包含 `AI 周报`
- 新闻事实应附原始来源链接，优先引用公司公告、产品文档、论文和监管机构等一手来源

自动任务会在写入文章前检查工作区、拉取远程更新，构建通过后只提交本次生成的文章并推送到 `main`。如果工作区存在未提交修改，任务会停止并报告，避免覆盖人工编辑。
